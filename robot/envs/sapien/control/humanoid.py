...
class HumanoidEnv(SapienEnv, utils.EzPickle):
    ...
    def build_model(self):
        ...
        torso = self.add_link(None,  Pose(np.array([0, 0, 1.4]), PxIdentity), "torso") # root coordinates #free
        self.add_capsule(torso, np.array([0, 0., 0]), x2y, 0.07, 0.07, default_rgb, "torso1")
        self.add_sphere(torso, np.array([0, 0., 0.19]), x2y, 0.09, default_rgb, "head")
        self.add_capsule(torso, np.array([-0.01, 0, -0.12]), x2y, 0.06, 0.06, default_rgb, "uwaist")
        ...
        lwaist_fake = self.add_link(torso,  Pose(np.array([0, 0, 0.]), PxIdentity), "lwaist_fake", "abdomen_z",
                                       sapien_core.ArticulationJointType.REVOLUTE, np.array([[np.radians(-45), np.radians(45)]]),
                                       joint_pos, joint_pos,
                                       ) # end in torso's framework
        joint_pos = parent_pose(*lwist_pos, np.array([0., 0., 0.065]), x2y)
        lwaist = self.add_link(lwaist_fake,  Pose(np.array([0, 0, 0.]), PxIdentity), "lwaist", "abdomen_y",
                                  sapien_core.ArticulationJointType.REVOLUTE, np.array([[np.radians(-75), np.radians(30)]]),
                                  joint_pos, Pose(np.array([0., 0., 0.065]), x2y)
                                  )
        # NOTE: the default of mujoco is rot in z direction, we need z2
        self.add_capsule(lwaist, np.array([0., 0, 0.]), x2y, 0.06, 0.06, default_rgb, "lwaist")


        joint_pos = np.array([0., 0., 0.1]), PxIdentity
        pelvis = self.add_link(lwaist,  Pose(np.array([0, 0, 0.]), PxIdentity), "pelvis", "abdomen_x",
                                  sapien_core.ArticulationJointType.REVOLUTE, np.array([[np.radians(-35), np.radians(35)]]),
                                  parent_pose(np.array([0, 0, -0.165]), np.array([1., 0, -0.002, 0]), *joint_pos),
                                  Pose(*joint_pos)
                              )
        self.add_capsule(pelvis, np.array([-0.02, 0., 0.]), x2y, 0.09, 0.07, default_rgb, "butt")
        ...
        for dir in ['right', 'left']:
            if dir == 'right':
                ...
                local_pos = Pose(np.array([0, 0.1, -0.04]), PxIdentity)
            hip_x = self.add_link(pelvis, Pose(np.array([0, 0, 0.]), PxIdentity), f"{dir}_hip_x", f"{dir}_hip_x",
                                     sapien_core.ArticulationJointType.REVOLUTE,
                                     np.array([[np.radians(-25), np.radians(5)]]),
                                     local_pos, Pose(np.array([0, 0, 0]), PxIdentity),
                                     )  # end in local pos
            hip_z_pos = Pose(np.array([0, 0, 0]), PxIdentity)
            hip_z = self.add_link(hip_x, Pose(np.array([0, 0, 0.]), PxIdentity), f"{dir}_hip_z", f"{dir}_hip_z",
                                     sapien_core.ArticulationJointType.REVOLUTE,
                                     np.array([[np.radians(-60), np.radians(35)]]),
                                     hip_z_pos, hip_z_pos)  # end in local pos

            hip_y_pos = Pose(np.array([0, 0, 0]), x2y)
            thigh = self.add_link(hip_z, Pose(np.array([0, 0, 0.]), PxIdentity), f"{dir}_thigh", f"{dir}_hip_y",
                                     sapien_core.ArticulationJointType.REVOLUTE,
            ...
            shin = self.add_link(th, Pose(np.array([0, 0, 0.]), PxIdentity), f"{dir}_shin", f"{dir}_knee",
                                     sapien_core.ArticulationJointType.REVOLUTE,
                                     np.array([[np.radians(-160), np.radians(-2)]]),
                                     parent_pose(*shin_pos, *local_pos), Pose(*local_pos),
                                     )  # end in local pos
            self.add_capsule(shin, np.array([0, 0, -0.15]), np.array([0.7071068, 0, -0.7071068, 0]), #x2-z
                             0.049, 0.15, default_rgb, f"{dir}_shin")
            self.add_sphere(shin, np.array([0, 0, -0.35]), PxIdentity, #x2-z
            ...
            shoulder_1 = self.add_link(torso, Pose(np.array([0, 0, 0.]), PxIdentity), f"{dir}_shoulder1", f"{dir}_shoulder1",
                                     sapien_core.ArticulationJointType.REVOLUTE,
            ...
            upper_arm = self.add_link(shoulder_1, Pose(np.array([0, 0, 0.]), PxIdentity), f"{dir}_upper_arm", f"{dir}_shoulder2",
                                          sapien_core.ArticulationJointType.REVOLUTE,
                                          np.array([[np.radians(-85), np.radians(60)]]),
            ...
            l, quat = vec2pose(vec)
            self.add_capsule(arm, vec, quat, 0.04, 0.16, default_rgb, f"{dir}_thigh1")
            ...
            if dir == 'right':
                ...
                arm_pose = np.array([0.18, 0.18, -0.18]), PxIdentity
            local_pos = np.array([0, 0, 0]), vec2pose(np.array([0, -1, -1]))[1]
            lower_arm = self.add_link(arm, Pose(np.array([0, 0, 0.]), PxIdentity), f"{dir}_lower_arm", f"{dir}_elbow",
                                          sapien_core.ArticulationJointType.REVOLUTE,
            ...
            self.add_capsule(lower_arm, middle, quat, 0.031, l, default_rgb, f"{dir}_lower_arm")
        ...
        self.add_force_actuator("abdomen_y", -100, 100)
        self.add_force_actuator("abdomen_x", -100, 100)
        for i in ['left', 'right']:
            self.add_force_actuator(f"{i}_hip_x", -100, 100)
            self.add_force_actuator(f"{i}_hip_y", -100, 100)
            self.add_force_actuator(f"{i}_hip_z", -300, 300)
            self.add_force_actuator(f"{i}_knee", -200, 200)
            self.add_force_actuator(f"{i}_shoulder1", -25, 25)
            self.add_force_actuator(f"{i}_shoulder2", -25, 25)
            self.add_force_actuator(f"{i}_elbow", -25, 25)

        ground = self.sim.add_ground(-1)
        torso = wrapper.get_links()[0]
        wrapper.set_root_pose(Pose(np.array([0, 0, 1.4]), PxIdentity))
