...
class SAC(object):
    def __init__(self, num_inputs, action_space, gamma=0.99, tau=0.005, alpha=0.2, policy='Gaussian',
                 ...
                 hidden_size=256, lr=0.0003, cuda=True):
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = GaussianPolicy(num_inputs, action_space.shape[0], hidden_size, action_space).to(
                self.device)
    ...
    def __call__(self, state, eval=False):
        return self.select_action(state, True)
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        mask_batch = torch.tensor(mask_batch, device=self.device, dtype=torch.float).unsqueeze(1)
