...
def main():
    ...
    env = make(env_name)
    recorder = RLRecorder(env, '/tmp/tmp/fetchreach', save_model=slice(100000000, None, 1), network_loss=slice(0, None, 50),
                          evaluate=slice(0, None, 50), save_video=1, max_timestep=timestep)
    DDPGAgent(
        n=8, num_epoch=2500, timestep=timestep, n_rollout=2, n_batch=n_batch,
