...
import numpy as np
from robot.torch_robotics.engine import Engine
import tqdm
...
def test_box():
    engine = Engine(dt=0.001, frameskip=10, collision_threshold=1e-3, mu=1., contact_dof=3, restitution=1)
    ground = engine.ground(10)
...
def test_sphere_box():
    engine = Engine(dt=0.005, frameskip=10, contact_dof=3, mu=1., restitution=0)
    ground = engine.ground(10)
...
def test_articulation():
    ...
    engine.add(*articulation)

    articulation[0].qpos[:] = tr.togpu([np.pi/2, 1])
